...
\end{equation*}
Die Ausgangsspannung ist phasengleich mit $U_\text{P}$, die am nicht-invertierenden Eingang ($+$) anliegt, und gegenphasig zu $U_\text{N}$, die am invertierenden Eingang ($-$) anliegt. Das Schaltbild eines Operationsverstärkers ist in \autoref{fig:schaltung} dargestellt.
...
\end{equation*}
Außerhalb dieses Bereichs ist die Ausgangsspannung $\pm U_\text{S}$. Dies ist in \autoref{fig:kennlinie} dargestellt. Dabei ist $U_{\text{S}}$ die Ausgangsspannung beim Schwellenwert, welche 
...
\label{eqn:tau}
\end{equation}
 die Abklingdauer und $T=2\pi RC$ die Schwingungsdauer.