...
\end{equation*}
Die Ausgangsspannung ist phasengleich mit $U_\text{P}$, die am nicht-invertierenden Eingang ($+$) anliegt, und gegenphasig zu $U_\text{N}$, die am invertierenden Eingang ($-$) anliegt. Dies ist in \autoref{fig:schaltung} dargestellt. Das dreieckige Symbol ist veraltet, das aktuelle Symbol ist ein Rechteck.

\begin{figure}%
    \centering%
    %\includegraphics[width=0.8\textwidth]{images/schaltbild_buch.png}%
    \caption{Schaltbild eines Operationsverstärkers. Die Betriebsspannung $U_\text{B}$ wird üblicherweise nicht dargestellt. \cite{BrabetzHaasSpieker+2015}}%
    \label{fig:schaltung}%
\end{figure}%
